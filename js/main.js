...
if (clientsEl.length > 0) {
  const clients = new Swiper('.clients-carousel', {
    // slidesPerView: 'auto', // Changed from 'auto'
    slidesPerView: 6,       // Set a fixed number (adjust 5, 6, 7, etc. based on preference)
    spaceBetween: 64,       // Spacing between logos
    // centeredSlides: true, // Changed from true
    centeredSlides: false,  // Ensure slides aren't centered (better for ticker)
    loop: true,             // Keep looping
    speed: 4000,            // Adjust speed if desired (lower number = faster scroll)
    allowTouchMove: false, // Disable manual swipe
    autoplay: {
      delay: 0,             // Delay of 0ms = continuous scroll
      disableOnInteraction: false, // Allow scrolling to continue after user interaction
    },
